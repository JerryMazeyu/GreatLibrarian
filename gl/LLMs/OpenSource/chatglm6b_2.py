from gl.Core import LLMs
import requests
import json
import configparser
from gl.LLMs.dbInteractor import get_db_connection, write_data_to_database
...
class ChatGLM6B_2(LLMs):
    ...
    def __call__(self, prompt: str) -> str:  # 这里输出改成了dict
        ...
        req = {"prompt": prompt, "history": []}
        payload = json.dumps(req)
        headers = {"Content-Type": "application/json"}

        response = requests.post(
            api_address + ":" + port, headers=headers, data=payload
        )
        if response.status_code == 200:
            data = {**req, **response.json()}
            data["llm_name"] = self.model
            db = get_db_connection(
                hostname=config.get("chatGLM6B_2_Local", "host"),
            ...
            write_data_to_database(
                db=db, tablename=config.get("chatGLM6B_2_Local", "tablename"), data=data
            )
