...
add_logger_to_class = add_logger_name_cls(log_name,
                                          os.path.join('Logs', logger_subfile))
...
class Analyse():
    ...
    def analyse(self):
        ...
        score_get = [0] * 10
        field_list = [
            'knowledge_understanding', 'coding', 'common_knowledge',
            'reasoning', 'multi_language', 'specialized_knowledge',
            'traceability', 'outputformatting', 'internal_security',
            'external_security'
        ]
        total_score = [0] * 10
    ...
    def report(self, plotinfo, log_path, llm_name,
               llm_intro):  #TODO:用图像展示以下测评结果：各领域题目回答准确率，各领域题目占比，测试题目总数，测试用时等
        """
        ...
        filtered_fields = [
            fields for fields, total_scores in zip(field, total_score)
            if total_scores > 0
        ]
        ...
        filtered_score_get = [
            score for score, total_scores in zip(score_get, total_score)
            if total_scores > 0
        ]
        ...
        patches, texts, autotexts = axs[1].pie(percentages,
                                               labels=filtered_fields,
                                               autopct='%1.1f%%',
                                               startangle=140)
