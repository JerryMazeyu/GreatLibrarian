        ...
        if final_score != "Human Evaluation":
            if float(final_score) <= 0.25:
                if "blacklist" in self.eval_info:
                    print(
                        f'Mistaken case:prompt:{self.prompt},ans:{ans_list},field:{self.field},keywords:{self.eval_info["keywords"][0]},blacklist:{self.eval_info["blacklist"][0]}'
                    )
                else:
                    print(
                        f'Mistaken case:prompt:{self.prompt},ans:{ans_list},field:{self.field},keywords:{self.eval_info["keywords"][0]}'
                    )
            if float(final_score) > 0.25:
                print(
