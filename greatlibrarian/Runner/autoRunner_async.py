from greatlibrarian.Utils import record_testcase
from greatlibrarian.Interactor import (
    AutoInteractor,
    HallucinationInteractor,
    SafetyInteractor,
    AutoInteractor_Async,
)
from greatlibrarian.TestCase import TestProject
import os
import json
import concurrent.futures
from tqdm import tqdm
from greatlibrarian.EvalMethods import (
    ToolUse,
    Keyword,
    LLMEval,
    Blacklist,
...
class AutoRunner_Async:
    ...
    async def run(self) -> None:
        ...
        num_done = 0
        record_testcase_process(
            f"目前的进度为：{num_done}/{self.testcase_num}", self.log_path
        )
        ...
        async def run_interactor(testproj, interactor_cls, cfg, methodnum, threadnum):
            ...
            for testcase in testproj.get_cases(cfg):
                with lock:
                    ...
                    dec = await setup_async(
                        logger_name="dialog_init", logger_file=self.log_path
                    )
                    # interactor = apply_decorator_to_all_methods(dec(), interactor)
                    interactor.run = await apply_decorator_to_func_async(
                        dec, interactor.run
                    )
                    ...
                    if interactor is not None:
                        ...
                        with num_done_lock:
                            num_done += 1
                            record_testcase_process(
                                f"目前的进度为：{num_done}/{self.testcase_num}",
                                self.log_path,
                            )
        ...
        await self.analyse(
            os.path.join(self.log_path, "dialog_init.log"), self.test_type
        )
        record_project_info(
            self.project_name,
            self.test_llm_name,
